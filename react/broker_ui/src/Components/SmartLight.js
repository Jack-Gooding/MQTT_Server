...
import { faPowerOff, faInfoCircle } from "@fortawesome/free-solid-svg-icons";
...
export default function SmartLight(props) {
  ...
  useEffect(() => {
    if (props.data.color.hue != null) {
      ...
      propColor = hsv_to_hsl(propColor.hue/65534,propColor.sat/254,propColor.bri/254);
      newColor[0] = propColor[0]*360;
      newColor[1] = (propColor[1]*254 > 200) ? (200-127)/(200-127)*85+15 : (propColor[1]*254 < 127) ? (127-127)/(200-127)*85+15 : (propColor[1]*254-127)/(200-127)*85+15 ;
      ...
      <div className="smart-light-card panel-card">
        <div className="card-title-bar">
          ...
          <div className="color-sliders">
            ...
            <Slider min="1" max="254" item="bri" defaultValue={props.data.color.bri} handleChange={e => updateLightState(e, "bri")} handleMouseUp={(e) => changeLightColor(e, "bri")}/>
          </div>
          <div className="color-block" style={{background: `${props.data.on ? `hsl(${color[0]},${color[1]}%,${color[2]}%)` : `hsla(${color[0]}, 24%, 15%, 1)`} ${props.data.on ? "" : `url(${hashlines})`}`}}></div>
      </div>
    );
};
