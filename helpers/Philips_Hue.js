...
const username = "sTCBhMMGDjoeyTJpufczCCfDK0KhQRy9ALaMn4E3";

// async function discoverBridge() { //NO LONGER WORKING
async function discoverBridgeLegacy() {
  //NO LONGER WORKING
  try {
    ...
    if (discoveryResults.length === 0) {
      ...
      console.log(
        `Found ${discoveryResults.length} bridge(s): ${discoveryResults[0].ipaddress}`
      );
    ...
    if (lightId != null) {
      ...
      if (light._data.type === "Extended color light") {
        state = new LightState()
          .transition(0)
          .hue(Math.random() * 65535)
          .sat(Math.random(254 / 2) + 254 / 2)
          .on();
      } else {
        ...
        for (let i = 0; i < lights.length; i++) {
          ...
          if (light._data.type === "Extended color light") {
            ...
            state = new LightState()
              ...
              .on();
            promises.push(
              hueBridge.lights.setLightState(light._data.id, state)
            );
          }
        }
